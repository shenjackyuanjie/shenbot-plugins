...
def fmt_info(show_sync: bool = False) -> str:
    ...
    if market_data is not None and star_data is not None:
        ...
        _ = cache.write(f"应用/元服务:{market_data['app_count']['total']}, 应用: {market_data['app_count']['apps']} 元服务: {market_data['app_count']['atomic_services']}\n")
...
def query_rank(msg: IcaNewMessage, client: IcaClient) -> None:
    ...
    top_down_info = api_helper("apps/list/1?page_size=5&desc=true&sort=download_count")
    ...
    top_down_info = api_helper("apps/list/1?page_size=5&desc=true&sort=download_count&exclude_huawei=true")
    if top_down_info is not None:
        top_down_data = top_down_info['data']['data']
        for idx, app in enumerate(top_down_data):
            release_date = datetime.datetime.fromtimestamp(app['release_date'] / 1000.0)
